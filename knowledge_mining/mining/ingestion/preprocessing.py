...
def _render_list(node: dict[str, Any], heading_offset: int, ordered: bool) -> str:
    ...
    for idx, item in enumerate(items, start=1):
        ...
        body = "".join(_render(c, heading_offset) for c in item.get("children", []))
        body = _inline(body)
        if body:
...
def convert_topic(html_path: Path, depth: int) -> str:
...
def convert_chm_extracted(extracted_dir: Path, doc_title: str | None = None) -> tuple[str, dict]:
    ...
    for depth, _hhc_title, local in entries:
        ...
        if not topic_path.exists():
            ...
            continue
        try:
            md = convert_topic(topic_path, depth)
...
def convert_hdx_extracted(extracted_dir: Path, doc_title: str | None = None) -> tuple[str, dict]:
...
def convert_extracted(
    ...
    *,
    ...
    if layout == "chm":
        return convert_chm_extracted(extracted_dir, doc_title)
    if layout == "hdx":
        return convert_hdx_extracted(extracted_dir, doc_title)
